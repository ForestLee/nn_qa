...
if __name__ == '__main__':
    ...
    s1_value = np.random.normal(0,1, [1,64,128])
    s2_value = np.random.normal(0,1, [1,64,128])
    s3_value = np.random.normal(0,1, [1,30,128])
    s1_tensor = tf.keras.backend.variable(s1_value)
    s2_tensor = tf.keras.backend.variable(s2_value)
    s3_tensor = tf.keras.backend.variable(s3_value)
    model = MatchingLayer(matching_type='dot')([s1_tensor, s2_tensor])
    ret1 = tf.keras.backend.eval(model)
    model = MatchingLayer(matching_type='dot')([s1_tensor, s3_tensor])
    ret2 = tf.keras.backend.eval(model)
    print("done")
