...
def load_vocab(dict_path, ner_vocab = False):
    """从Bert词典文件中读取词典
    """
    token_dict = {}
    ...
    #for NER vocab
    #if pretrain_vocab == True:
    token_dict["unknow"] = len(token_dict)    #add unkown,
    for token in ["[PAD]", "[CLS]", "[SEP]", "[UNK]", "[MASK]"]:
        if token not in token_dict.keys():
            token_dict[token] = len(token_dict)

    print("vocab len "+str(len(token_dict)))
    return token_dict
...
class Tokenizer(BasicTokenizer):
    ...
    """
    def __init__(self, token_dict, case_sensitive=True, ner_vocab = False):
        """初始化
        """
        super(Tokenizer, self).__init__()
        if is_string(token_dict):
            token_dict = load_vocab(token_dict, ner_vocab)
